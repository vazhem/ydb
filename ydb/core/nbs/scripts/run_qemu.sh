...
-b, --background               Run in background and detach
--log-file                     Log file path (used with --background, default: /tmp/qemu-<diskid>.log)
--ssh-port                     SSH port for QEMU VM (default: 8679)
--qmp-port                     QMP port for QEMU management (default: 8678)
EOF
}
...
ssh_port=8679
qmp_port=8678
options=$(getopt -l "help,key:,diskid:,socket:,encryption-key-path:,encrypted,background,log-file:,ssh-port:,qmp-port:" -o "hk:d:s:eb" -a -- "$@")
...
#blockstore-client startendpoint --ipc-type vhost --socket $socket --client-id client-1 --instance-id localhost --disk-id $diskid 
#sleep 1

# run qemu with secondary disk
# Ports are now set from command-line arguments (parsed above)
...
if [ $background -eq 1 ]; then
    echo "Starting qemu in background with disk [$diskid]"
    ...
    sudo $QEMU \
        ...
        -daemonize \
        -pidfile /tmp/qemu-$diskid.pid
    
    if [ $? -eq 0 ]; then
        echo "QEMU started successfully"
        echo "PID: $(cat /tmp/qemu-$diskid.pid 2>/dev/null || echo 'unknown')"
        echo "To view logs: tail -f $log_file"
        echo "To stop: sudo kill \$(cat /tmp/qemu-$diskid.pid)"
    else
